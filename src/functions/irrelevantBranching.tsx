import { AnswerOption, AnswerItem, CodeTree, Code, Variable } from "@/app/types";
import { addAnnotationsFromAnswer } from "./mapAnswersToAnnotations";
import { Question, Answer } from "@/app/types";

export function processIrrelevantBranching(unit: any, questions: Variable[], answers: Answer[], questionIndex: number) {
  ...
  for (let a of answers) {
    ...
    for (let value of a.makes_irrelevant) {
      ...
      const i = questions.findIndex((q: Variable) => q.name === value);
      if (i >= 0) which.add(i);
    }
  }
...
export const getMakesIrrelevantArray = (items: AnswerItem[], options: Code[]): string[] => {
