...
const SelectRelationPage = ({ edge, annotationLib, annotationManager, setOpen }: SelectRelationPageProps) => {
  const onSelect = useCallback(
    (value: CodeSelectorValue, ctrlKey: boolean) => {
      if (value.cancel) {
        setOpen(false);
        ...
        if (value.code) annotationManager.createRelationAnnotation(value.code, edge.from, edge.to);
      }
  ...
  return <PopupSelection header={`${edge.from.value} → ${edge.to.value}`} options={options} onSelect={onSelect} />;
};
...
function getOptions(from: Annotation[], to: Annotation[], variable: Variable) {
  ...
  for (let f of from) {
    if (!f.variable || !f.value) continue;
    const fromRelations = validFrom?.[f.variable]?.["*"] || validFrom?.[f.variable]?.[f.value] || null;
    if (!fromRelations) continue;

    for (let t of to) {
      if (f.id === t.id) continue;
      if (!t.variable || !t.value) continue;
      ...
      if (!edgeRelations[key]?.relationOption) {
        ...
        edgeRelations[key].relationOption.relations = [...edgeRelations[key].relationOption.relations, ...relations];
      }
    }
  }
