"use client";
import { useCodebooks } from "@/app/api/projects/[projectId]/codebook/query";
...
export default function Job({ params }: { params: { projectId: number } }) {
  ...
  return (
    <div className="mx-auto mt-10 flex max-w-xl flex-col gap-2 p-3">
      <Button asChild>
        <Link href={`/manage/${params.projectId}/users`}>Users</Link>
      </Button>
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="max-w-72">
        <DBSelect {...useCodebooksProps} nameField={"name"} projectId={projectId}>
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              variant="secondary"
              onClick={() => create(newName).then(({ id }) => router.push(`/manage/${projectId}/codebook/${id}`))}
            >
              <Plus />
            </Button>
          </div>
