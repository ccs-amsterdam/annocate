...
export default function UserRoleTable({ user, ownRole, users, roles, changeRole }: Props) {
  ...
  function onChangeRole(email: string, currentRole: string, newRole: string) {
    if (currentRole === newRole) return;
    if (email === user?.email && roleHigherThan(currentRole, newRole, roles)) {
...
function roleHigherThan(currentRole: string, newRole: string, roles: string[]): boolean {
  return roles.indexOf(currentRole) < roles.indexOf(newRole);
}
