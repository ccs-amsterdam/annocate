import { Variable, TokenSelection, ValidTokenRelations, ValidTokenDestinations, AnnotationLibrary } from "@/app/types";

export const getValidTokenRelations = (annotationLib: AnnotationLibrary, variable: Variable): ValidTokenRelations => {
  ...
  for (let sa of startAnnotations) {
    ...
    for (let key of Object.keys(annotationLib.byToken)) {
      ...
      for (let destinationId of destinationAnnotationIds) {
        ...
        if (da.variable === sa.variable && da.value === sa.value && da.offset === sa.offset) continue;
        valid[i] = true;
      }
    }
  }
