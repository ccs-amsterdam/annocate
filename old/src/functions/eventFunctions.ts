import { SetState, Token, TokenSelection, Mover, TriggerSelector, Arrowkeys } from "@/app/types";
...
export function onKeyUp(
  ...
  setMover: SetState<Mover | null>,
  triggerSelector: TriggerSelector,
) {
  if (event.key === " " && holdSpace) {
    setHoldSpace(false);
    if (tokenSelection && tokenSelection.length > 0) {
...
export function storeMouseTokenSelection(
  ...
  setTokenSelection((state: TokenSelection) => updateSelection(state, tokens, currentNode.index, true, sameFieldOnly));
...
export function onTouchUp(
  ...
  tokenSelection: TokenSelection,
  sameFieldOnly: boolean,
  setCurrentToken: (i: number | null) => void,
  setTokenSelection: SetState<TokenSelection>,
  triggerSelector: TriggerSelector,
  touch: any,
) {
  ...
  if (tokenSelection && tokenSelection.length > 0 && tokenSelection[0] === currentToken) {
    ...
    const newTokenSelection = updateSelection(tokenSelection, tokens, currentNode, true, sameFieldOnly);

    if (newTokenSelection && newTokenSelection[0] !== currentNode && newTokenSelection[1] !== currentNode) {
...
export function onMouseMove(
  ...
  if (!editMode && selectionStarted.current) {
    ...
    storeMouseTokenSelection(getToken(tokens, event), tokens, sameFieldOnly, setCurrentToken, setTokenSelection);
  } else {
...
export function onMouseUp(
  ...
  if (tokenSelection && tokenSelection.length > 0 && tokenSelection[0] !== null && tokenSelection[1] !== null) {
...
export const movePosition = (
  ...
  sameFieldOnly: boolean,
  setCurrentToken: (i: number) => void,
  setTokenSelection: SetState<TokenSelection | null>,
) => {
  ...
  if (mover.position !== newPosition) {
    ...
    setTokenSelection((state: TokenSelection | null) => {
...
const annotationFromSelection = (tokens: Token[], selection: TokenSelection, triggerSelector: TriggerSelector) => {
...
const moveToken = (tokens: Token[], key: Arrowkeys, space: boolean, mover: Mover, sameFieldOnly: boolean) => {
  ...
  if (key === "ArrowRight" || key === "ArrowDown") {
    const nextAnnotation = tokens.findIndex(
      (token, i) =>
        ...
        (token?.ref?.current.classList.contains("allLeft") || token?.ref?.current.classList.contains("anyLeft")),
    );
...
const moveSentence = (tokens: Token[], mover: Mover, direction: "up" | "down" = "up") => {
...
const updateSelection = (
  ...
  if (index === null || selection === null) return selection;
