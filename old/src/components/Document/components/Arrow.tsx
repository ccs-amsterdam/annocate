...
export default function Arrow({
  ...
  if (!tokenSelection || tokenSelection.length !== 2 || tokenSelection[1] === null) return;

  const elX = tokens[tokenSelection[0]]?.ref?.current;
  const elY = tokens[tokenSelection[1]]?.ref?.current;

  const elXend = tokenSelectionEnd && tokenSelectionEnd[0] ? tokens[tokenSelectionEnd[0]]?.ref?.current : elX;
  const elYend = tokenSelectionEnd && tokenSelectionEnd[1] ? tokens[tokenSelectionEnd[1]]?.ref?.current : elY;
  ...
  function getXY(p: typeof p1, pend: typeof p1, x: string, y: string) {
    ...
    return [pend.x - xoffset, pend.y + pend.h + yoffset];
  }
